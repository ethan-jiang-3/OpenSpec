...
### 项目用了 agent/graph 时：config 只记录稳定 authority map，不接管运行时

有模型参与不代表应该把 prompt、节点顺序、状态、权限、重试和 recovery 全写进 `config.yaml`。先问**谁拥有 Flow 和最终事实**，再把只有所有 planning artifacts 都需要的、稳定的边界浓缩进 context：

| 运行时形态 | context 可以稳定说明什么 | 仍必须归谁 |
|---|---|---|
| 传统确定性程序主导 | 用户产品边界、代码/数据/行为合同的权威位置、长期质量目标 | 程序、tests、CI 负责真实流程和验证 |
| MD/Agent 主导的流程 | controller、确定性 gate、状态/receipt 与人类授权分别由谁拥有 | playbook、agent contract、gate/checker 负责具体步骤 |
| Program/Graph 主导的流程 | graph、node contract、state schema、evaluator 等 source of truth 在哪里 | graph/runtime、typed state、tests 负责转移与恢复 |

这不是给 config 增加 `flow_owner:`、`agentic_mode:` 等新字段——它们不会被 OpenSpec 消费。它的目的只是让 proposal/specs/design 作者知道该向谁取证、不能把哪段 prompt prose 当第二份控制流真相。一次 change 的实际分类、选路和风险仍留在 proposal/design/tasks；不可绕过的规则仍由 schema、runtime contract、checker 或 CI 承担。
...
| capability path、Purpose、关键词、相邻边界的薄索引 | `specs/README.md` 或独立 catalog | 帮人和 agent 找到正确 main spec；和 main spec 冲突时以后者为准 |
| 角色权限行为 | `specs/` | 系统承诺 |
...
### 再加一个容易漏掉的问题：它是“行为真相”还是“导航信息”？

项目增长后会出现第三类信息：例如 `identity/session` 负责什么、关键词是什么、和 `identity/login` 的边界在哪里。这些信息能帮助 agent 先选对候选 capability，却不应该复制完整 requirements。

- **完整行为、场景、SHALL/MUST**：仍然只放 main spec；这是事实层。
- **path + 一句 Purpose + 关键词 + 邻居**：可放薄 catalog；这是导航层。
- **每次都必须遵守的发现纪律**：放 `AGENTS.md` 或 `rules.proposal`，例如“先查 catalog，再把受影响 path 标为 New 或 Modified”。

绝不要把 catalog 当第二份 spec，也不要把全表塞进 `context`。前者会和 archive 后的 main spec 漂移，后者会变成每次 prompt 都注入的过期清单。

---

## 到这里为止：本章只判断信息归位

本章的权威问题是“这条信息属于 `config.yaml`、main spec、change、catalog，还是 schema”。一旦已经判定它该进入 `config.yaml`，字段怎么写、规则怎么变得可判断、怎样验证不同 consumer 是否真的读到它，统一由 [06](06-高级-config-yaml-怎么写到真正好用.md) 负责。

下面保留的 `config.yaml` 内容只用于说明**为什么它属于项目层**，不是可复制配置的权威范本。实际落地时，优先以 06 的字段路由、样例和 `instructions --json` 验证步骤为准。
...
v1.8.0 把 Apply/Archive 的短、稳定项目步骤放在 `operations.apply.guidance` 与 `operations.archive.guidance`。它们与 project `context` 进入对应 operation instructions；不要误写成 `rules.apply` / `rules.archive`。这些字段不改变 schema gate、也不替代测试/CI。
...
这四类分开以后，`config.yaml` 会更清楚：artifact rules 管生成，operations 管 Apply/Archive，schema 管结构。

---

## 一个用于判断归位的 `config.yaml` 轮廓

下面这份偏企业级 Web 项目轮廓只说明“哪些类型适合进入项目层”；需要直接采用的 YAML 范本时，回到 06，而不是维护两份标准答案。
...
rules:
  proposal:
    - Identify authorization, audit, and compatibility risks; record the required controls and verification evidence.
  ...
  design:
    ...
    - Domain modules live under src/<domain>/; shared UI belongs in src/ui/.
    - State the integration/e2e test locations relevant to the design.
  specs:
...
这一节只比较项目长期约束的侧重点，不定义字段写法或 consumer 路由。
...
rules:
  proposal:
    - Identify audit and sensitive-action risks and record the required controls.
  specs:
...
- **本次具体动作**（放 `changes/<name>/tasks.md`）：
  ...
  - [ ] Run existing auth tests — verify: auth test command exits 0
  - [ ] Add 2 new tests for new approval path — verify: both tests fail before the fix and pass after it
  - [ ] Verify the complete regression suite — verify: required CI check is green
  ```
