...
| 正式能力层 | `openspec/specs/` | 当前系统正式承认自己会什么？ | 合同 / API 文档 |
| 单次变更层 | `openspec/changes/<change-id>/` | 这次 change 准备改什么、怎么改？ | PR 描述 / 工作包 |

只要这三层不混，OpenSpec 的很多设计都会变得非常清楚。

> **本章讨论的是单仓库场景**——绝大多数项目用这三层就够。如果你管理的是一个跨多个仓库的大项目，全局约束还可以往上提到 workspace 层（v1.4.0），让跨仓库的规划原则不散落在各 repo 里。那是更高级的话题，详见 [`07`](07-高级-workspace-跨仓库规划.md)。
...
如果一条规则满足下面两条中的大多数，它通常更适合进 `config.yaml`：
...
> **`config.yaml` 应该像项目宪法，不应该像项目仓库的回收站。**
...
### 不该只靠 `config.yaml` 的第二类：单次变更的具体动作
