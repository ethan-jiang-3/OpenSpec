...
把 Apply/Archive 的短、稳定项目步骤放在 `operations.apply.guidance` 与 `operations.archive.guidance`。它们与 project `context` 进入对应 operation instructions；不要误写成 `rules.apply` / `rules.archive`。这些字段不改变 schema gate、也不替代测试/CI。
