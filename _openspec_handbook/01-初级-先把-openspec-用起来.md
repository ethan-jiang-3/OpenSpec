...
> **v1.10.0 使用提示。** 本章把 `/opsx:*` 保留为 Claude 的示例入口；Codex 使用 `$openspec-*`，Zed Agent 使用 `/openspec-*` 或 `@openspec-*`。Codex、Zed 与 vendor-neutral `agents` 目标共享 `.agents/skills/`，都是宿主 workflow 驱动同一套 `openspec` CLI / 文件状态，不要把 slash 命令当作所有工具的统一语法。
...
不要从代码目录或页面名字直接猜 spec 文件。一个 **capability** 是能独立解释、独立修改、独立验证的行为合同；它往往跨 UI、服务和存储。v1.8.0（v1.7.0 起）允许把这种合同放在嵌套 path：
...
### 新项目想让 artifacts 使用中文或其他语言

新项目可在第一次初始化时同时指定工具和 artifact 语言：

```bash
openspec init --tools zed --language "简体中文"
```

这会创建类似下面的合法 `openspec/config.yaml`：
...
这里本地化的是 proposal、design、task 描述和 requirement prose；`## ADDED Requirements`、`### Requirement:`、`#### Scenario:` 等结构 heading，以及规范关键词 `SHALL` / `MUST` 保持英文，否则 parser/validator 可能无法按预期识别。

`--language` 是**新 config 的种子**，不是覆盖命令。如果项目已有 `openspec/config.yaml`，CLI 会失败并保持原文件不变：
...
上面的 `--tools zed` 会生成 `.agents/skills/openspec-*/SKILL.md`。Zed Agent 是 skills-only，不会生成 `opsx-*` command；需 Zed ≥ 1.4.2，并先信任当前 worktree。通常用 `/openspec-propose` 或 `@openspec-propose` 调用。
