...
> **v1.8.0 当前模型。** capability ID 是 `openspec/specs/` 下的相对 path，允许 `identity/session/spec.md` 这样的嵌套；这是路径命名空间，不是继承树、自动聚合或自动挑选相关 main specs（v1.7.0 引入，v1.8.0 不变）。下面的 `/opsx:*` 均是 Claude 示例，Codex 使用 `$openspec-*` skills（v1.8.0 下装在 `.agents/skills/`）。
...
- `openspec/` 里大多是项目事实层
- `.claude/`、`.agents/`（Codex 与 vendor-neutral `agents` 目标共享，v1.8.0）、`.cursor/` 这类更多是工具入口层
