...
**现在不用纠结它**——只要先知道：`spec-driven` 是 OpenSpec 默认的 schema，也是**背后的 driver**（每个 change 的 artifact 骨架、delta 操作、能力契约都由它定义）。想真正吃透它，看 [`09-高级-能力身份与specs漂移维护`](09-高级-能力身份与specs漂移维护.md) 的开头，以及项目根 `_digested/schema/02-内置-spec-driven-详解.md`。
