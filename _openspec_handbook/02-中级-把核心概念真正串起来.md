...
合并是**按目录名寻址**的：delta 在 `changes/<name>/specs/<capability>/`，打到主 spec 里**同名**的 `openspec/specs/<capability>/`。所以一个 capability 的"地址"就是它的目录名——这也是为什么 capability 目录名不能乱改（详见 [`09`](09-高级-能力身份与specs漂移维护.md)）。
