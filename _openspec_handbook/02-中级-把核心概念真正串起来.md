...
> **v1.7.0 当前模型。** capability ID 是 `openspec/specs/` 下的相对 path，允许 `identity/session/spec.md` 这样的嵌套；这是路径命名空间，不是继承树、自动聚合或自动挑选相关 main specs。下面的 `/opsx:*` 均是 Claude 示例，Codex 使用 `$openspec-*` skills。
