...
- **三层递进梯度**：装修这类生活比喻是帮没概念的读者建立直觉的"拐杖"，随读者爬升平滑淡出，不一直赖着——**初级浓**（生活类比建立直觉、概念最少、图多）、**中级淡**（仅在核心概念处短锚点呼应、不大段铺陈；概念精选讲透、不面面俱到吓走读者；图多）、**高级隐去**（读者已建立软件开发心智模型，用纯概念/设计动机论述，按需用 graph/gantt/架构图）。各篇装修浓度据此定位，不各写各的。
