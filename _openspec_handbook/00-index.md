...
- **手册版本**（v1.0）：本手册自身的版次。理解加深、内容大修时升版。
...
| [09-高级-能力身份与specs漂移维护.md](09-高级-能力身份与specs漂移维护.md) | 能力身份模型（capability=目录名）+ specs 漂移维护 | 想搞懂 specs 怎么组织、为什么会漂、怎么守的人 |
...
> 本手册是**一篇一篇增量补出来的**。为保证各篇定位与风格统一、不"各写各的"，新写/改写都遵循下面约束。"这一版的组织原则"（见下）规定**叙事顺序**（先用→再懂→最后机制）；本节是**体裁与风格的硬约束**，二者配合。
...
3. 以 `## 下一步` / `## 继续读` 收尾，且**必须是最后一节**——不得在它之后再追加内容节（版本补丁应并入正文或上提到开篇）。
...
- **图种**：主力 `graph`/`sequenceDiagram`，按场景灵活选择 `gantt`（时间线/阶段对比）、`mindmap`（概念全景/脑图）、`gitGraph`（Git 分支流程）；同篇同种 ≤2 张、勿与文字流程列表重复；**选图种的第一原则是"这个场景用它最直观"，不是"别的篇没用过所以不能用"**。
