...
- **手册版本**（v1.6）：本手册自身的版次。理解加深、内容大修时升版。
...
- Codex 在 v1.8.0 使用 skills-only 投递，入口是 `$openspec-*` skill（装在 `.agents/skills/`，与 vendor-neutral `agents` 目标共享根），而不是 `/opsx:*` command。
...
先在 09 建立“行为合同切片、稳定 path、catalog 只导航”的判断，再按需要深入 [`_digested/spec-driven-capability/`](../_digested/spec-driven-capability/README.md)。嵌套目录可被 v1.8.0 完整处理（v1.7.0 起），但它不会替 agent 自动挑选相关 specs。
