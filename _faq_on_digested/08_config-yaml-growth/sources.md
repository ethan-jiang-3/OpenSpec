...
源码引用以 v1.7.0 tag `4e16790` 为当前基线。`src/core/project-config.ts` 还解析 `operations`；Apply/Archive instructions 会接收 project `context` 和对应 `operations.*.guidance`。下方任何只列 context/rules、或说 Apply 不接收 config 的旧来源说明，均以这个当前边界为准。
