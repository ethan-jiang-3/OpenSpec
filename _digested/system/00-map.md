...
- 用 store/reference/workset 模型支持多仓库上下文引用。
...
这五层不是替代关系，而是叠加关系。repo-local 仍然是单仓库工作的默认形态；store 不是把所有 repo 合并成一个大 repo，而是通过声明的 reference 关系让 agent 知道「还有哪些仓库的 specs 可以看」。
