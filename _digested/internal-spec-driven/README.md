# internal — OpenSpec 源码机制深挖

这个目录是 `_digested/` 的"内核层"。`_digested/` 的其他子目录（`spec_full_content/`、`schema/`、`spec_cli/`）侧重**概念解释和用法指导**，而 `internal/` 聚焦于**从源码出发的精确机制剖析**。
...
| **`internal/`** | **从 TypeScript 源码出发的精确机制剖析** | **想彻底搞懂每条命令背后到底发生了什么的人** |
...
所有文件中的引用都精确到 TypeScript 源文件的函数名和行号（基于当前 `ethan` 分支的源码状态）。关键源文件总览见 `05` 第 9 节。
