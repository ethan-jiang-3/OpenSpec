...
用户/AI 通过 workflow 选择一个 change，change 使用某个 schema，schema 定义 artifact graph，CLI 根据 graph 和文件状态推导 completed/readiness，再通过 instructions 把 template/context/rules 编译成下一步执行包。
